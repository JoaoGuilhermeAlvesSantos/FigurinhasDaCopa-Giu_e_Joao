...
def sort_sticker_codes(codes: list[str]) -> list[str]:
    import re
...
def build_duplicate_teams(results: dict) -> list[dict]:
    all_stickers = results["all_stickers"]
    grouped: dict[str, list[str]] = {code: [] for code in TEAM_META}

    for sticker in all_stickers:
        team_code = sticker.get("group")
        code = sticker.get("code")
        if team_code in grouped and code:
            grouped[team_code].append(code)
    ...
    for code, meta in TEAM_META.items():
        teams.append({
            ...
            "group": meta["group"],
            "isHost": meta["isHost"],
            "stickers": sort_sticker_codes(grouped[code]),
            "total": len(grouped[code]),
        })
    return teams
...
def generate(results_path: str = "results.json", out_path: str = "docs/data.js") -> None:
    ...
    duplicate_data = {
        ...
        "teams": build_duplicate_teams(raw),
    }
    ...
    duplicate_out_path = Path(out_path).parent / "duplicate_data.js"
    js_duplicate = f"// Auto-generated by generate_site_data.py – {date.today()}\n"
    js_duplicate += "const ALBUM_DUPLICATE_DATA = "
    js_duplicate += json.dumps(duplicate_data, ensure_ascii=False, indent=2)
    js_duplicate += ";\n"
    with open(duplicate_out_path, "w", encoding="utf-8") as f:
        f.write(js_duplicate)

    print(f"[OK] Written to {out_path}  ({len(js_content):,} bytes)")
    print(f"[OK] Written to {duplicate_out_path}  ({len(js_duplicate):,} bytes)")
